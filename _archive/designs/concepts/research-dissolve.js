...
export function createResearchDissolve(carousel) {
  ...
  const document = carousel.ownerDocument, view = document.defaultView;
  const reduced = view.matchMedia('(prefers-reduced-motion: reduce)');
  const sourceCache = new Map(), sampleCache = new Map(), imageRequests = new Set();
  ...
  let active = null, disposed = false, inView = true, serial = 0;
  ...
  function sourceOf(img) { return img?.src || img?.currentSrc || ''; }
  ...
  function loadImage(element) {
    ...
    const promise = new Promise(resolve => {
      ...
      image.onload = () => {
        ...
        if (typeof image.decode === 'function') image.decode().catch(() => {}).then(() => finish(image));
        else finish(image);
      };
    ...
    promise.then(result => { if (!result) sourceCache.delete(src); });
    return promise;
  }
  ...
  function positionFraction(token, freeSpace) {
    ...
    if (!token || token === 'center') return freeSpace * .5;
  ...
  function measure(element, decoded, host) {
    const box = element.getBoundingClientRect(), hostBox = host.getBoundingClientRect();
    ...
    return {
      elementBox, content, draw, image: decoded,
  ...
  function sampleFigure(image, metrics, count, src) {
    ...
    const scale = Math.min(1, draw.width * 1.35 / image.naturalWidth, draw.height * 1.35 / image.naturalHeight, 900 / image.naturalWidth, 700 / image.naturalHeight);
    ...
    const random = seeded(key), points = new Array(count);
    const step = visible.length / count;
    for (let index = 0; index < count; index++) {
      const pixel = visible[Math.min(visible.length - 1, Math.floor((index + random()) * step))];
      const offset = pixel * 4;
      const r = pixels[offset], g = pixels[offset + 1], b = pixels[offset + 2], alpha = pixels[offset + 3] / 255;
      points[index] = {
        x: (pixel % width + .5) / width,
        y: (Math.floor(pixel / width) + .5) / height,
        r, g, b, alpha, color: 'rgb(' + r + ',' + g + ',' + b + ')',
      };
    }
  ...
  function cancel() { if (active) settle(active, true, true); }

  function placeCanvas(state, host) {
    const hostBox = host.getBoundingClientRect(), carouselBox = carousel.getBoundingClientRect();
  ...
  function drawFrame(context, metrics) {
    ...
    if (metrics.background && metrics.background !== 'rgba(0, 0, 0, 0)' && metrics.background !== 'transparent') {
      context.fillStyle = metrics.background; context.fill();
    }
  ...
  function drawImage(context, metrics, opacity) {
    ...
    const content = metrics.content, draw = metrics.draw;
    context.beginPath(); context.rect(content.x, content.y, content.width, content.height); context.clip();
  ...
  function render(state, progress) {
    ...
    const frameBlend=smooth((progress-.22)/.56);
    const frameBox={};
    for(const key of ['x','y','width','height'])frameBox[key]=state.fromMetrics.elementBox[key]+(state.toMetrics.elementBox[key]-state.fromMetrics.elementBox[key])*frameBlend;
    drawFrame(context,{...(progress<.5?state.fromMetrics:state.toMetrics),elementBox:frameBox});
    drawImage(context, state.fromMetrics, 1 - smooth(progress / .24));
    const assemble = smooth((progress - .53) / .36);
    const disperse = smooth((progress - .035) / .425);
    const particleOpacity = smooth(progress / .10) * (1 - smooth((progress - .88) / .12));
    const colorBlend = smooth((progress - .38) / .26);
    const curl = Math.sin(progress * Math.PI) ** 1.4;
    for (let index = 0; index < state.particles.length; index++) {
      const particle = state.particles[index], source = state.fromPixels[index], target = state.toPixels[index];
      ...
      const cloudX = state.width * .5 + ((startX + endX) * .5 - state.width * .5) * .58 + particle.dx * state.width * .25;
      const cloudY = state.height * .5 + ((startY + endY) * .5 - state.height * .5) * .52 + particle.dy * state.height * .29;
      let x = progress < .5 ? startX + (cloudX - startX) * disperse : cloudX + (endX - cloudX) * assemble;
      let y = progress < .5 ? startY + (cloudY - startY) * disperse : cloudY + (endY - cloudY) * assemble;
      x += Math.sin(progress * Math.PI * 2 + particle.phase) * particle.wave * curl;
      y += Math.cos(progress * Math.PI * 1.65 + particle.phase) * particle.wave * curl * .70;
      if (colorBlend <= 0) context.fillStyle = source.color;
      else if (colorBlend >= 1) context.fillStyle = target.color;
      else context.fillStyle = 'rgb(' + Math.round(source.r + (target.r - source.r) * colorBlend) + ',' + Math.round(source.g + (target.g - source.g) * colorBlend) + ',' + Math.round(source.b + (target.b - source.b) * colorBlend) + ')';
      context.globalAlpha = particleOpacity * (source.alpha + (target.alpha - source.alpha) * colorBlend) * particle.opacity;
      const size = particle.size * (1 + Math.sin(progress * Math.PI) * .19);
      context.fillRect(x - size * .5, y - size * .5, size, size);
    }
    context.globalAlpha = 1;
    drawImage(context, state.toMetrics, smooth((progress - .83) / .17));
  }

  async function prepare(state) {
    try {
      const fromImage = figure(state.fromSlide), toImage = figure(state.toSlide);
      const fromHost = fromImage?.closest('.pc-enlarge'), toHost = toImage?.closest('.pc-enlarge');
      if (!fromHost || !toHost) { settle(state); return; }
      if (document.documentElement.dataset.theme === 'dark' &&
          [fromImage, toImage].some(image => image.dataset.themeAsset !== 'dark')) {
        settle(state); return;
      }
      ...
      if (!fromDecoded || !toDecoded || reduced.matches || paused() || !inView || document.hidden) { settle(state); return; }
      if (typeof toImage.decode === 'function') { try { await toImage.decode(); } catch {} }
      if (active !== state) return;
      const fromMetrics = measure(fromImage, fromDecoded, fromHost);
      const toMetrics = measure(toImage, toDecoded, toHost);
      if (fromHost.getBoundingClientRect().width < 2 || fromMetrics.draw.width < 2 || toMetrics.draw.width < 2) { settle(state); return; }
      ...
      if (!fromSample || !toSample) { settle(state); return; }
      const fromPixels = fromSample.points, toPixels = toSample.points;
      ...
      if (!context) { settle(state); return; }
      const random = seeded(sourceOf(fromImage) + '>' + sourceOf(toImage));
      state.particles = Array.from({length: count}, () => ({
        dx: random() - .5, dy: random() - .5, phase: random() * Math.PI * 2,
        wave: 3 + random() * (mobile ? 9 : 15),
        size: (mobile ? .95 : 1.05) + random() * .85,
        opacity: .68 + random() * .32,
      }));
      Object.assign(state, {canvas, context, fromMetrics, toMetrics, fromPixels, toPixels, fromImage, toImage, fromHost, toHost, toDecoded, toBitmap: toSample.bitmap});
      ...
      for (const element of [fromImage, toImage]) {
        if (!element.classList.contains('pc-dissolve-hidden')) {
          element.classList.add('pc-dissolve-hidden'); state.hidden.add(element);
        }
      }
      ...
      function frame(now) {
        if (active !== state) return;
        if (!carousel.isConnected || document.hidden || reduced.matches || paused() || !inView) { settle(state, true, true); return; }
        const progress = clamp((now - start) / duration);
        carousel.dataset.dissolveProgress = progress.toFixed(3);
        try {
          if (progress >= .5 && !state.committed) {
  ...
  function transition(fromSlide, toSlide, commit) {
    ...
    return new Promise(resolve => {
      const state = {
        id: ++serial, fromSlide: interruptedTarget || fromSlide, toSlide, commit, resolve,
        committed: false, started: false, frame: 0, hidden: new Set(),
      };
      active = state;
      prepare(state);
    });
  ...
  function onResize() { sampleCache.clear(); cancel(); }
  function onVisibility() { if (document.hidden) cancel(); }
  function onReduced() { if (reduced.matches) cancel(); }
  ...
  const resizeObserver = typeof view.ResizeObserver === 'function' ? new view.ResizeObserver(() => {
    ...
    if (Math.abs(width - observedWidth) > 1) { observedWidth = width; onResize(); }
  }) : null;
  resizeObserver?.observe(carousel);
  const intersectionObserver = typeof view.IntersectionObserver === 'function' ? new view.IntersectionObserver(entries => {
  ...
  function dispose() {
    ...
    sourceCache.clear(); sampleCache.clear();
