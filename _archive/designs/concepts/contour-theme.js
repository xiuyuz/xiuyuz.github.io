...
const icons = {
  light: '<svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.35" stroke-linecap="round" aria-hidden="true"><circle cx="10" cy="10" r="3.3"/><path d="M10 1.5v1.7m0 13.6v1.7M1.5 10h1.7m13.6 0h1.7M4 4l1.2 1.2m9.6 9.6L16 16M4 16l1.2-1.2m9.6-9.6L16 4"/></svg>',
  ...
  wrapper.innerHTML = `
    ...
    <div class="ct-theme-popover" id="${id}" hidden>
      <fieldset class="ct-theme-options" aria-describedby="${id}-hint">
        ...
        ${MODES.map(mode => `<label class="ct-theme-option"><input type="radio" name="${id}-mode" value="${mode}"><span>${LABELS[mode]}</span></label>`).join('')}
      </fieldset>
