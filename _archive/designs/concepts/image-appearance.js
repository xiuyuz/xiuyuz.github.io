import {getDarkFigure, disposeDarkFigures} from './figure-theme.js?v=dark-assets-8';
...
export function mountImageAppearance(root) {
  ...
  const entries = new Map(), lifecycle = new AbortController();
  const darkSources = new Map();
  let disposed = false, printing = false;
  ...
  async function apply(entry) {
    ...
    try {
      ...
      if (!disposed && entry.image.isConnected && entries.get(entry.image) === entry &&
          entry.version === version && isDark() && !originalRequested(entry)) {
  ...
  function applyAll() { return Promise.all([...entries.values()].map(apply)); }
  root.querySelectorAll('.pc-enlarge img, .ct-school-logo').forEach(image => remember(image));

  root.addEventListener('ct-figure-open', event => {
  ...
  view.addEventListener('beforeprint', () => {printing = true; applyAll();}, {signal: lifecycle.signal});
  view.addEventListener('afterprint', () => {printing = false; applyAll();}, {signal: lifecycle.signal});
