import {createResearchSignals} from './particle-signals.js?v=finish-6';
...
export function createParticleStudy(THREE,distribution){
  ...
  const compact=distribution.compact||matchMedia('(max-width:760px)').matches;
  ...
  const grainScale=distribution.compact?tuning.previewGrain||1:compact?tuning.mobileGrain||1:tuning.desktopGrain||1;
  const pose=distribution.rotation||[.20,-.35,-.08];
  const rotation=new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(...pose));
  const samplePoint=new THREE.Vector3();
  const centers=[new THREE.Vector3(),new THREE.Vector3(),new THREE.Vector3()],regionCounts=[0,0,0];
  const samples=Array.from({length:count},(_,i)=>{
    ...
    const position=sample.position,region=sample.region||0;
    centers[region].add(samplePoint.fromArray(position));regionCounts[region]++;
    samplePoint.applyMatrix4(rotation);
    return{position,region,size:sample.size||1,density:sample.density||.9,depth:samplePoint.z,phase:rand()*TAU};
  });
  centers.forEach((center,i)=>center.divideScalar(regionCounts[i]||1));
  samples.sort((a,b)=>a.depth-b.depth);
  const positions=[],sizes=[],phases=[],regions=[],densities=[],strays=[];
  for(const sample of samples){
    positions.push(...sample.position);sizes.push((1.30+Math.pow(rand(),4)*1.35)*sample.size);
    phases.push(sample.phase);regions.push(sample.region);densities.push(sample.density);
    strays.push(sample.region===1&&sample.depth>.05&&rand()>.985?.6+rand()*.4:0);
  }
  const colors=(distribution.colors||['#3f6089','#7e69a2','#428b91']).map(c=>new THREE.Color(c).convertLinearToSRGB());
  const uniforms={
    uTime:{value:0},uNight:{value:0},uDpr:{value:Math.min(devicePixelRatio||1,distribution.compact?1:1.75)},
    uScale:{value:(distribution.compact?1.0:compact?.92:1.13)*grainScale},uSafety:{value:0},
    uFocus:{value:new THREE.Vector3()},uLight:{value:new THREE.Vector3(-.6,.8,1.2)},
    uPointer:{value:new THREE.Vector3(0,0,0)},uCenters:{value:centers},uColors:{value:colors}
  };
  const geometry=new THREE.BufferGeometry();
  for(const[name,values,size]of[['position',positions,3],['aSize',sizes,1],['aPhase',phases,1],['aRegion',regions,1],['aDensity',densities,1],['aSafety',strays,1]])geometry.setAttribute(name,new THREE.Float32BufferAttribute(values,size));
  geometry.computeBoundingSphere();
  const material=new THREE.ShaderMaterial({uniforms,vertexShader,fragmentShader,transparent:true,depthWrite:false,depthTest:true,blending:THREE.NormalBlending,toneMapped:false});
  const cloud=new THREE.Points(geometry,material);
  const object=new THREE.Group();object.add(cloud);object.rotation.set(...pose);
  const signals=createResearchSignals(THREE,samples,pose);object.add(signals.object);
  if(distribution.compact)signals.object.traverse(child=>{if(child.material?.uniforms?.uPixelRatio)child.material.uniforms.uPixelRatio.value=.72;});
  const focusWeights=[0,0,0];
  const update=(time,pointer={})=>{
    uniforms.uTime.value=time;
    const phase=(time+1)/10%1;
    uniforms.uSafety.value=smooth((phase-.10)/.5)*(1-smooth((phase-.88)/.12));
    focusWeights.forEach((value,i)=>focusWeights[i]=value+(Number(pointer.focus===i+1)-value)*.10);
    ...
    uniforms.uLight.value.set(-.6+(pointer.x||0)*.85,.8-(pointer.y||0)*.40,1.2);
    object.rotation.set(pose[0]+Math.sin(time*.18)*.09,pose[1]+Math.sin(time*.16)*.34,pose[2]+Math.sin(time*.13)*.045);
    signals.update(time,pointer);
    if(distribution.compact)signals.object.traverse(child=>{if(child.material?.uniforms?.uOpacity)child.material.uniforms.uOpacity.value*=.72;});
  };
  const setTheme=theme=>{uniforms.uNight.value=theme==='dark'?1:0;signals.setTheme?.(theme);};
