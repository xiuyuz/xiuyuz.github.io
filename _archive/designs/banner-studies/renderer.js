...
// Full previews and visible gallery cards share the same animation lifecycle.
// Compact scenes use a smaller pixel buffer and can be controlled externally.
export async function mountArtwork(root, study, options = {}) {
  ...
  let renderer, scene, camera, environment, art, pivot, key, fill;
  let disposed = false, contextLost = false, failed = false, inView = true;
  let paused = false, motionOverride = false;
  let frame = 0, last = 0, time = 0;
  let targetX = 0, targetY = 0, lightX = 0, lightY = 0;
  let targetArtX = 0, targetArtY = 0, targetInfluence = 0;
  let hoveredInterest = 0, focusedInterest = 0;
  ...
  const state = () => failed || contextLost ? 'static'
    : paused ? 'paused'
      : reduced.matches && !motionOverride ? 'reduced'
        : document.hidden ? 'hidden' : !inView ? 'offscreen' : 'running';
  ...
  function draw(dt = 0) {
    ...
    pointer.x = lightX; pointer.y = lightY;
    ...
    if (renderer && !contextLost && art) {
      ...
      pivot.rotation.y = lightX * .045;
      pivot.rotation.x = lightY * .025;
  ...
  function resize() {
    ...
    if (renderer && !contextLost && camera && bounds.width > 0 && bounds.height > 0) {
      ...
      const fit = Math.max(modelSize.y, modelSize.x / camera.aspect) * .59 / tangent;
      const tuning = study.rendering || {};
      const enlargement = options.compact ? tuning.previewScale || 1 : innerWidth <= 760 ? tuning.mobileScale || 1 : tuning.desktopScale || 1;
      camera.position.set(0, 0, (fit + modelSize.z * .47) * (bounds.width < 500 ? .90 : .78) / enlargement);
  ...
  function tick(now) {
    frame = 0;
    if (disposed) return;
    if (state() !== 'running') { sync(); return; }
    if (!last) last = now;
    if (now - last >= 1000 / (options.compact ? 24 : 30) - .6) {
  ...
  function sync() {
    ...
    if (toggle) {
      ...
      toggle.title = reduced.matches && !motionOverride
        ? 'Reduced motion is enabled. Select Play motion to animate this artwork.' : '';
  ...
  function onPointer(event) {
    ...
    targetInfluence = x >= 0 && x <= 1 && y >= 0 && y <= 1 ? 1 : .24;
  }

  try {
    renderer = new THREE.WebGLRenderer({
      canvas, alpha: true, antialias: true, powerPreference: 'low-power',
      preserveDrawingBuffer: !options.compact,
    });
    renderer.setPixelRatio(Math.min(devicePixelRatio || 1, options.compact ? 1 : 1.75));
    renderer.setClearColor(0xffffff, 0);
    renderer.toneMapping = THREE.ACESFilmicToneMapping;
    renderer.toneMappingExposure = study.exposure || 1.06;
    scene = new THREE.Scene();
    camera = new THREE.PerspectiveCamera(study.fov || 32, 1, .1, 100);
    if (!study.particles) {
    ...
    key = new THREE.DirectionalLight(0xfffefe, 2.7); scene.add(key);
    fill = new THREE.DirectionalLight(0xd7e4ff, 1.25); scene.add(fill);
    const rim = new THREE.DirectionalLight(0xffffff, 1.3);
    rim.position.set(1, -1, -4); scene.add(rim);
    ...
    if (disposed) { art.dispose?.(); return () => {}; }
    ...
    const center = bounds.getCenter(new THREE.Vector3()); bounds.getSize(modelSize);
    const offset = new THREE.Group();
    offset.position.copy(center).negate(); offset.add(art.object);
    pivot = new THREE.Group(); pivot.add(offset); scene.add(pivot);
    resize(); stage.classList.add('is-ready'); panel.dataset.renderer = 'webgl';
  } catch (error) {
    failed = true; canvas.hidden = true; panel.dataset.renderer = 'static';
    console.error('Artwork preview:', error);
  }

  const onTheme = () => {
    const theme=document.documentElement.dataset.theme || 'light';
    art?.setTheme?.(theme);stage.dataset.theme=theme;draw();
  };
  ...
  const onReduced = () => {
    ...
    targetX = targetY = 0; clearPointer(); sync();
  };
  const onVisibility = () => {
  ...
  const onLost = event => {
    ...
    stage.classList.remove('is-ready'); panel.dataset.renderer = 'static'; sync();
  };
  const onRestored = () => {
    contextLost = false;
    stage.classList.add('is-ready'); panel.dataset.renderer = 'webgl'; resize(); sync();
  };
  const updateFocus = () => {
  ...
  const interestHandlers = [...panel.querySelectorAll('.ct-interest')].map((interest, index) => {
    if (!options.compact) interest.tabIndex = 0;
    const enter = () => { hoveredInterest = index + 1; updateFocus(); };
    const leave = () => { if (hoveredInterest === index + 1) hoveredInterest = 0; updateFocus(); };
    const focus = () => { focusedInterest = index + 1; updateFocus(); };
    const blur = event => {
  ...
  const dispose = () => {
    if (disposed) return;
    disposed = true; interestHandlers.forEach(cleanup => cleanup());
    ...
    art?.dispose?.(); environment?.dispose(); renderer?.dispose(); renderer?.forceContextLoss();
  };
