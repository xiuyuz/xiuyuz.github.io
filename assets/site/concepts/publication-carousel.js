...
import { createResearchDissolve } from "./research-dissolve.js";
...
function renderAbstract(paper, index) {
  ...
  if (expandable && !curated) {
    ...
    if (!/[.!?]$/.test(excerpt)) {
      ...
      excerpt = sentenceEnd > excerpt.length * 0.65 ? excerpt.slice(0, sentenceEnd + 1) : `${excerpt}…`;
    }
  }
  const id = `pc-abstract-${index}`;
  return `<div class="pc-abstract"><span class="pc-abstract-label">${
    paper.abstract ? "Abstract" : "Overview"
  }</span><div class="pc-abstract-body" id="${id}"><p data-pc-excerpt>${escape(excerpt)}</p>${
    expandable ? `<p data-pc-full hidden>${escape(text)}</p>` : ""
  }</div>${
    expandable
      ? `<button class="pc-abstract-toggle" type="button" data-pc-abstract-toggle aria-expanded="false" aria-controls="${id}"><span data-pc-abstract-label>Read full abstract</span><span data-pc-abstract-symbol aria-hidden="true">+</span></button>`
      : ""
  }</div>`;
}
...
export function renderCarousel(papers = []) {
  ...
  return `<div class="publication-carousel" role="region" aria-roledescription="carousel" aria-label="Research highlights" tabindex="0" data-current="0" data-transition="particles">
    <div class="pc-heading"><span>RESEARCH, IN FOCUS</span><div class="pc-heading-tools"><button class="pc-effects-toggle" type="button" data-pc-effects-toggle data-paused="false" aria-pressed="false" aria-label="Pause research effects"><svg class="pc-effects-pause" width="10" height="12" viewBox="0 0 10 12" aria-hidden="true"><path d="M3 2v8M7 2v8" fill="none" stroke="currentColor" stroke-width="1.3"/></svg><svg class="pc-effects-play" width="10" height="12" viewBox="0 0 10 12" aria-hidden="true"><path d="m2 2 6 4-6 4Z" fill="currentColor"/></svg><span class="pc-effects-label">Pause effects</span></button><span class="pc-counter" aria-hidden="true"><span data-pc-current>01</span><span class="pc-counter-rule">/</span>${String(
      count
    ).padStart(2, "0")}</span></div></div>
    <div class="pc-window"><div class="pc-track">${papers
      .map(
        (paper, index) => `<article class="pc-slide" role="group" aria-roledescription="slide" aria-label="${
          index + 1
        } of ${count}" data-pc-index="${index}" data-paper-key="${escape(paper.key)}"${index ? ' inert aria-hidden="true"' : ""}>
      <div class="pc-feature${
        paper.abstract || paper.summary ? "" : " pc-feature-figure-only"
      }"><button class="pc-enlarge" type="button" data-figure="${escape(paper.image)}" data-title="${escape(
        paper.title
      )}" aria-label="Enlarge figure from ${escape(paper.title)}"><img src="${escape(paper.image)}" alt="Research figure from ${escape(
        paper.title
      )}" loading="${
        index ? "lazy" : "eager"
      }" decoding="async" draggable="false"><span class="pc-enlarge-label" aria-hidden="true"><svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M10 2h4v4M14 2 9 7M6 14H2v-4M2 14l5-5" stroke="currentColor" stroke-width="1.2"/></svg>Enlarge figure</span></button>${renderAbstract(
        paper,
        index
      )}</div>
      <div class="pc-caption"><h3><a href="${escape(paper.url)}" target="_blank" rel="noopener noreferrer">${escape(
        paper.title
      )}</a></h3><div class="pc-caption-footer"><div class="pc-meta">${renderVenue(paper)}</div><a class="pc-details" href="#selected-paper-${escape(
        paper.key
      )}">Research details <span aria-hidden="true">↓</span></a></div></div>
    </article>`
      )
      .join("")}</div></div>
    ${
      count > 1
        ? `<div class="pc-controls"><div class="pc-selectors" role="group" aria-label="Choose a research figure">${papers
            .map(
              (paper, index) =>
                `<button class="pc-picker${index ? "" : " is-current"}" type="button" data-pc-goto="${index}" aria-label="Show figure ${
                  index + 1
                } of ${count}: ${escape(paper.title)}"${index ? "" : ' aria-current="true"'}><span>${String(index + 1).padStart(
                  2,
                  "0"
                )}</span></button>`
            )
...
export function mountCarousel(root = document) {
  ...
  for (const carousel of carousels) {
    ...
    const commit = (index) => {
      ...
      const focusClass = document.activeElement?.closest("[data-pc-abstract-toggle]")
        ? "[data-pc-abstract-toggle]"
        : document.activeElement?.classList.contains("pc-details")
          ? ".pc-details"
          : document.activeElement?.closest("h3")
            ? "h3 a"
            : ".pc-enlarge";
      current = index;
      ...
      if (dissolve && carousel.classList.contains("pc-dissolve-active") && !reduced.matches) {
        slides[current]
          .querySelectorAll(".pc-abstract,.pc-caption")
          .forEach((copy) => copy.animate([{ opacity: 0 }, { opacity: 1 }], { duration: 300, easing: "ease-out" }));
      }
    ...
    disposers.push(() => {
      ...
      observer.disconnect();
      dissolve?.dispose();
      delete carousel.dataset.pcMounted;
    });
  }
