...
export function renderResearch(data) {
  const selected = (data.selectedPaperKeys || data.papers.slice(0, 4).map((p) => p.key))
    .map((key) => data.papers.find((p) => p.key === key))
    .filter(Boolean);
  ...
  return `<section class="ct-research ct-carousel-research" id="ct-research" aria-labelledby="ctr-heading">
    <div class="ct-section-heading ct-research-heading"><div><span class="ct-label">02 / RESEARCH</span><h2 id="ctr-heading">Research</h2></div><a class="ct-all-papers" href="${escape(
      data.scholarUrl
    )}" ${external}>All research <span aria-hidden="true">↗</span></a></div>
    ...
    <ol class="ctr-publications">${selected
      .map(
        (p, i) =>
          `<li class="ctr-publication" id="selected-paper-${escape(p.key)}"><span class="ctr-number" aria-hidden="true">${String(i + 1).padStart(
            2,
            "0"
          )}</span><div class="ctr-citation"><div class="ctr-meta">${renderVenue(p)}</div><h4><a href="${escape(p.url)}" ${external}>${escape(
            p.title
          )}</a></h4><p class="ctr-authors">${escape(p.authors).replaceAll(
            "XiuYu Zhang",
