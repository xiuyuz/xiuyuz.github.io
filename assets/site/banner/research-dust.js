...
export function mountResearchDust(root, study) {
  ...
  const [rx, ry, rz] = study.rotation || [0.2, -0.35, -0.08];
  const particles = Array.from({ length: 2100 }, (_, i) => {
    const sample = study.sample(rand, i, 2100),
  ...
  const cells = [...carousel.querySelectorAll(".pc-enlarge")].map((host) => {
    ...
    canvas.className = "study-research-dust";
  ...
  const onTheme = () => {
    colors =
      document.documentElement.dataset.theme === "dark" ? ["140,175,207", "171,152,194", "125,185,180"] : ["91,119,160", "145,123,172", "87,142,147"];
    paint();
  };
  function paint() {
    const angle = Math.sin(time * 0.12) * 0.24;
    ...
    for (const cell of cells) {
      ...
      for (const sample of particles) {
        ...
        const x = w * 0.53 + px * w * 0.52 * depth;
        const y = h * 0.57 - (p[1] + Math.sin(time * 0.16 + p[0] * 2) * 0.016) * h * 0.41 * depth;
        if (x < 0 || x > w || y < 0 || y > h) continue;
        if (mask && x > mask.left && x < mask.right && y > mask.top && y < mask.bottom) continue;
        const alpha = (0.11 + 0.1 * Math.max(0, Math.min(1, (pz + 1.5) / 3))) * (sample.density || 0.8);
        ctx.fillStyle = `rgba(${colors[sample.region || 0]},${alpha.toFixed(3)})`;
        const size = 0.8 + depth * 0.4;
  ...
  function resize() {
    ...
    for (const cell of cells) {
      const bounds = cell.host.getBoundingClientRect();
      cell.width = bounds.width;
      cell.height = bounds.height;
      cell.canvas.width = Math.round(bounds.width * dpr);
      cell.canvas.height = Math.round(bounds.height * dpr);
      cell.context.setTransform(dpr, 0, 0, dpr, 0, 0);
      const figure = cell.host.querySelector("img").getBoundingClientRect();
      cell.mask = {
        left: figure.left - bounds.left - 3,
        right: figure.right - bounds.left + 3,
        top: figure.top - bounds.top - 3,
        bottom: figure.bottom - bounds.top + 3,
      };
    }
    paint();
  }
  function tick(now) {
    frame = 0;
    if (disposed || paused || reduced.matches || document.hidden || !inView) return;
    if (!last) last = now;
    if (now - last > 1000 / 24) {
  ...
  function sync() {
    ...
    button.setAttribute(
      "aria-label",
      reduced.matches ? "Research motion disabled by reduced motion preference" : still ? "Play research particles" : "Pause research particles"
    );
    button.querySelector(".pc-effects-label").textContent = reduced.matches ? "Motion reduced" : still ? "Play particles" : "Pause particles";
    paint();
    if (!still && !document.hidden && inView) frame = requestAnimationFrame(tick);
  }
  const onPause = () => {
    paused = !paused;
    sync();
  ...
  cells.forEach((cell) => size.observe(cell.host));
  const view = new IntersectionObserver((entries) => {
    inView = entries[0].isIntersecting;
    sync();
  });
  view.observe(carousel);
  const slides = new MutationObserver(() => {
    resize();
    sync();
  });
  slides.observe(carousel, { subtree: true, attributes: true, attributeFilter: ["inert"] });
  ...
  return () => {
    ...
    view.disconnect();
    slides.disconnect();
    button.removeEventListener("click", onPause);
