...
export function createResearchSignals(THREE, samples, pose = [0, 0, 0]) {
  ...
  const radius = Math.max(0.45, radii[Math.floor((radii.length - 1) * 0.91)]);
  ...
  function sprites(count, name) {
    ...
    for (const [name, values, width] of [
      ["position", positions, 3],
      ["aColor", colors, 3],
      ["aSize", sizes, 1],
      ["aOpacity", opacities, 1],
    ]) {
  ...
  let pool = candidates.filter((candidate) => candidate.view.z > viewCenter.z - radius * 0.12 && candidate.position.distanceTo(center) < radius);
  if (pool.length < 20) pool = candidates;
  let seed = pool[0];
  let seedScore = Infinity;
  for (const candidate of pool) {
    const score = candidate.position.distanceToSquared(center) - (candidate.view.z - viewCenter.z) * radius * 0.18;
    if (score < seedScore) {
      seed = candidate;
      seedScore = score;
    }
  ...
  while (selected.length < Math.min(16, pool.length)) {
    ...
    for (const candidate of pool) {
      ...
      for (const existing of selected) {
        const dx = candidate.view.x - existing.view.x,
          dy = candidate.view.y - existing.view.y;
        const dz = candidate.view.z - existing.view.z;
        nearest = Math.min(nearest, dx * dx + dy * dy + dz * dz * 0.22);
      }
  ...
  const lineMaterial = new THREE.ShaderMaterial({
    uniforms: { uColor: { value: reasoningColor }, uOpacity: { value: 0.7 } },
    vertexShader: `
  ...
  const pulseCount = edges.length ? 4 : 0,
    pulseLength = 7;
  ...
  const safety = Array.from({ length: safetyCount }, (_, index) => {
    ...
    const end = anchor
      .clone()
      .lerp(center, 0.16)
      .addScaledVector(front, radius * 0.08);
    const outward = anchor.clone().sub(center).normalize();
    if (outward.lengthSq() < 0.1) outward.set(Math.cos(index * 1.9), Math.sin(index * 1.9), 0.2).normalize();
    const side = new THREE.Vector3().crossVectors(outward, front).normalize();
    const start = end
      .clone()
      .addScaledVector(outward, radius * (0.33 + index * 0.021))
      .addScaledVector(side, radius * 0.11);
    const controlA = start
      .clone()
      .addScaledVector(side, radius * 0.1)
      .addScaledVector(front, radius * 0.035);
    const controlB = end
      .clone()
      .addScaledVector(outward, radius * 0.105)
      .addScaledVector(side, -radius * 0.045);
    return { curve: new THREE.CubicBezierCurve3(start, controlA, controlB, end), outward, side, period: 7.2 + index * 0.53, phase: index * 0.193 };
  });
  ...
  for (const candidate of candidates) {
    regionalMeans[candidate.region].add(candidate.position);
    regionalCounts[candidate.region]++;
  }
  regionalMeans.forEach((mean, region) => (regionalCounts[region] ? mean.divideScalar(regionalCounts[region]) : mean.copy(center)));
  const meansOverlap = regionalMeans.some((mean, i) => regionalMeans.some((other, j) => j > i && mean.distanceTo(other) < radius * 0.36));
  const hubs = regionalMeans.map((mean, region) => {
    const theta = (region * TAU) / 3 + 0.38;
    const preferred = new THREE.Vector3(Math.cos(theta) * radius * 0.57, Math.sin(theta) * radius * 0.48, radius * 0.12)
      .applyQuaternion(inversePose)
      .add(center);
    const target = meansOverlap ? preferred : mean.clone().addScaledVector(front, radius * 0.11);
    const regional = candidates.filter((candidate) => candidate.region === region && candidate.position.distanceTo(center) < radius * 1.06);
    let closest = regional[0] || candidates[0],
      best = Infinity;
    ...
    return closest.position.clone().lerp(target, 0.12);
  });
  const routes = hubs.map((start, index) => {
    ...
    const controlA = start
      .clone()
      .lerp(end, 0.33)
      .addScaledVector(side, radius * 0.14)
      .addScaledVector(front, radius * 0.025);
    const controlB = start
      .clone()
      .lerp(end, 0.69)
      .addScaledVector(side, radius * 0.1);
    return { curve: new THREE.CubicBezierCurve3(start, controlA, controlB, end), side, period: 5.5 + index * 0.47 };
  });
  const packetsPerRoute = 2,
    dustPerPacket = 6,
    hubDustCount = 5;
  ...
  function update(time = 0, pointer = {}) {
    ...
    for (let index = 0; index < 3; index++) {
      ...
      additionalTime[index] += dt * focus[index] * 0.27;
    }
    ...
    for (let index = 0; index < nodeCount; index++) {
      ...
      movingAnchors[index].x += Math.sin(time * 0.36 + phase) * radius * 0.022;
      movingAnchors[index].y += Math.cos(time * 0.29 + phase) * radius * 0.018;
      movingAnchors[index].z += Math.sin(time * 0.33 + phase * 0.8) * radius * 0.024;
    }
    for (let index = 0; index < edges.length; index++) {
      ...
      lineAlpha[index * 2] = lineAlpha[index * 2 + 1] = 0.55 + 0.1 * Math.sin(reasoningTime * 0.68 + index * 1.7) ** 2;
    }
    for (let pulse = 0; pulse < pulseCount; pulse++) {
      const raw = reasoningTime / (2.45 + pulse * 0.37) + pulse * 0.213;
      ...
      const visibility = smooth(phase / 0.1) * smooth((1 - phase) / 0.1);
      for (let trail = 0; trail < pulseLength; trail++) {
        const u = phase - trail * 0.036;
        scratch.copy(movingAnchors[a]).lerp(movingAnchors[b], clamp(u));
        const opacity = u < 0 ? 0 : visibility * (1 - trail / pulseLength) ** 1.3 * (0.91 + focus[0] * 0.09);
        reasoningTraces.write(
          pulse * pulseLength + trail,
          scratch,
          reasoningPulse,
          (trail === 0 ? 10.5 : 5.7 - trail * 0.18) * (1 + focus[0] * 0.3),
          opacity
        );
      }
      lineAlpha[edgeIndex * 2] += visibility * 0.38;
      lineAlpha[edgeIndex * 2 + 1] += visibility * 0.3;
      nodeActivity[b] = Math.max(nodeActivity[b], smooth((phase - 0.72) / 0.22) * smooth((1 - phase) / 0.035));
    }
    for (let index = 0; index < nodeCount; index++) {
      temporaryColor.copy(reasoningColor).lerp(palette[selected[index].region], 0.23);
      reasoningNodes.write(
        index,
        movingAnchors[index],
        temporaryColor,
        (8 + nodeActivity[index] * 1.5) * (1 + focus[0] * 0.3),
        0.7 + focus[0] * 0.16 + nodeActivity[index] * 0.14
      );
    }
    lineMaterial.uniforms.uOpacity.value = (0.7 + focus[0] * 0.2) * (1 - 0.18 * Math.max(focus[1], focus[2]));
    lineGeometry.attributes.position.needsUpdate = true;
    lineGeometry.attributes.aAlpha.needsUpdate = true;
    reasoningNodes.commit();
    reasoningTraces.commit();

    for (let index = 0; index < safetyCount; index++) {
      const packet = safety[index];
      const phase = fract(safetyTime / packet.period + packet.phase);
      const travel = smooth((phase - 0.13) / 0.62);
      const visibility = smooth(phase / 0.075) * smooth((1 - phase) / 0.12);
      const alignment = smooth((travel - 0.43) / 0.55);
      temporaryColor.copy(coral).lerp(teal, alignment);
      for (let dust = 0; dust < safetyDustCount; dust++) {
        const progress = clamp(travel - dust * 0.009 * (1 - alignment));
        packet.curve.getPoint(progress, scratch);
        const theta = dust * 2.399 + safetyTime * (0.16 + dust * 0.008);
        const scatter = radius * (0.026 + (1 - alignment) * 0.02) * (dust ? 0.55 + dust * 0.1 : 0.22);
        scratch.addScaledVector(packet.side, Math.cos(theta) * scatter);
        scratch.addScaledVector(front, Math.sin(theta) * scatter);
        scratch.addScaledVector(packet.outward, Math.sin(theta * 1.7) * scatter * 0.45);
        safetySprites.write(
          index * safetyDustCount + dust,
          scratch,
          temporaryColor,
          (dust ? 5.5 + (dust % 3) * 0.32 : 11) * (1 + focus[1] * 0.3),
          visibility * (0.86 + focus[1] * 0.14) * (dust ? 0.64 : 1)
        );
      }
    }
    ...
    for (let routeIndex = 0; routeIndex < routes.length; routeIndex++) {
      ...
      for (let packet = 0; packet < packetsPerRoute; packet++) {
        const phase = fract(modalityTime / route.period + packet * 0.5 + routeIndex * 0.19);
        for (let dust = 0; dust < dustPerPacket; dust++) {
          const u = phase - dust * (0.021 + (dust % 2) * 0.004);
          const progress = clamp(u);
          route.curve.getPoint(progress, scratch);
          const theta = dust * 2.399 + packet * 1.1 + modalityTime * 0.29;
          const scatter = radius * (0.018 + (dust % 3) * 0.006) * Math.sin(progress * Math.PI);
          scratch.addScaledVector(route.side, Math.cos(theta) * scatter);
          scratch.addScaledVector(front, Math.sin(theta) * scatter);
          temporaryColor.copy(palette[routeIndex]).lerp(palette[(routeIndex + 1) % 3], smooth((progress - 0.58) / 0.4));
          const visibility = u < 0 ? 0 : smooth(progress / 0.075) * smooth((1 - progress) / 0.08);
          const opacity = visibility * (0.84 + focus[2] * 0.16) * (1 - dust / (dustPerPacket + 2));
          modalitySprites.write(
            (routeIndex * packetsPerRoute + packet) * dustPerPacket + dust,
            scratch,
            temporaryColor,
            (dust === 0 ? 10.5 : 5.7 - dust * 0.16) * (1 + focus[2] * 0.3),
            opacity
          );
        }
      }
    ...
    hubs.forEach((hub, index) => {
      for (let dust = 0; dust < hubDustCount; dust++) {
        const phase = dust * 2.399 + index * 0.7;
        scratch.copy(hub);
        scratch.x += Math.cos(phase + modalityTime * 0.12) * radius * 0.034 * (dust ? 1 : 0.12);
        scratch.y += Math.sin(phase + modalityTime * 0.12) * radius * 0.03 * (dust ? 1 : 0.12);
        scratch.z += Math.sin(phase * 1.5) * radius * 0.022;
        const breathe = 0.92 + Math.sin(modalityTime * 1.15 + index * 2.1) * 0.08;
        modalitySprites.write(
          transportCount + index * hubDustCount + dust,
          scratch,
          palette[index],
          (dust ? 4.7 : 8.5) * (1 + focus[2] * 0.3),
          breathe * (0.61 + focus[2] * 0.2) * (dust ? 0.67 : 1)
        );
      }
    });
