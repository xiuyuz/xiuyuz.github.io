...
function renderAbstract(paper, index) {
  ...
  const expandable = words.length > 95;
  let excerpt = text;
  if (expandable) {
...
export function renderCarousel(papers = []) {
  ...
  return `<div class="publication-carousel" role="region" aria-roledescription="carousel" aria-label="Research highlights" tabindex="0" data-current="0">
    ...
    <div class="pc-window"><div class="pc-track">${papers.map((paper, index) => `<article class="pc-slide" role="group" aria-roledescription="slide" aria-label="${index + 1} of ${count}" data-pc-index="${index}"${index ? ' inert aria-hidden="true"' : ''}>
...
export function mountCarousel(root = document) {
  ...
  for (const carousel of carousels) {
    ...
    carousel.addEventListener('click', event => {
      ...
      const abstractToggle = event.target.closest('[data-pc-abstract-toggle]');
      if (abstractToggle) {
      ...
      if (arrow) show(current + Number(arrow.dataset.pcDirection));
    }, true);
    ...
    mountResearchMotion(carousel);
    // Percentage-based translation remains aligned on resize without measurements.
  }
}
